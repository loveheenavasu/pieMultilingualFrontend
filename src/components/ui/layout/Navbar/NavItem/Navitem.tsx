...
const Navitem = ({ setImgSrc }: any) => {
    ...
    const fetchData = async () => {
        try {
            const response = await fetch("https://piemultilingualbackend.onrender.com/header");
            if (!response.ok) {
    ...
    const searchBar = async () => {
        try {
            const response = await fetch("https://piemultilingualbackend.onrender.com/setting");
            const data = await response.json();
            ...
            {windowWidth > 770 ? (
                <div>
                            ...
                            {menuData?.data?.map((item, index) => (
                                <>
                                    <div className='items-link'>
                                        {index == 0 && <img src={`https://piemultilingualbackend.onrender.com/${item.icon}`} alt="im" />}
                    ...
                    <div className="search-wrapper">
                        {isSearch && <><input className='search-input' placeholder='Search' />
                        ...
                        <div className="container-item">
                            <div className="nav-link">
                                ...
                                >
                                    ...
                                    {menuData?.data?.map((item, index) => (
                                        <>
                                            <MenuItem key={item?.name} selected={item.name === 'Pyxis'} onClick={handleClose}>
                                                <div className='items-link'>
                                                    {index == 0 && <img src={`https://piemultilingualbackend.onrender.com/${item.icon}`} alt="im" />}
                                                    <a href={item.link}>{item.name}</a>
                                                </div>
                            ...
                            <IconButton
                                ...
                                onClick={handleClick}
                            >
                                <img className='hamburger-icon' src="/images/hamburger.png" alt="search" />
                            </IconButton>
                        </div>
                    </>)}
            <div>
